...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    // 优先使用 GAZEBO_MODEL_PATH 查找 MID360 模型目录
    const char* model_path_env = std::getenv("GAZEBO_MODEL_PATH");
    if (model_path_env) {
        std::string model_path(model_path_env);
        std::istringstream iss(model_path);
        std::string path;
        while (std::getline(iss, path, ':')) {
            std::string candidate = path + "/MID360/scan_mode/mid360.csv";
            struct stat st;
    ...
    // 回退到 SDF 参数
    if (file_name.empty() && sdf->HasElement("csv_file_name")) {
        std::string csv_name = sdf->Get<std::string>("csv_file_name");
        const char* home_env = std::getenv("HOME");
        if (home_env) {
            file_name = std::string(home_env) + "/prometheus_px4/Tools/sitl_gazebo/models/MID360/scan_mode/" + csv_name;
        }
    }
    // 最终回退到硬编码路径
    if (file_name.empty()) {
        file_name = "/home/amov/prometheus_px4/Tools/sitl_gazebo/models/MID360/scan_mode/mid360.csv";
    }
