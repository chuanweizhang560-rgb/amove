...
void RealSensePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  // 注意：Gazebo include子模型后，sensor名字变成scoped name（如 "model::D435i::depth"），
  // 所以需要用suffix匹配而不是精确匹配
  for (int i = 0; i < sensors.size(); ++i)
  {
    const auto &sensor = sensors[i];
    std::string sensorName = sensor->Name();
    bool isDepth = (sensorName == prefix + DEPTH_CAMERA_NAME) ||
                   (sensorName.find("::" + std::string(DEPTH_CAMERA_NAME)) != std::string::npos &&
                    sensorName.find(_model->GetName()) != std::string::npos);
    bool isIred1 = (sensorName == prefix + IRED1_CAMERA_NAME) ||
                   (sensorName.find("::" + std::string(IRED1_CAMERA_NAME)) != std::string::npos &&
                    sensorName.find(_model->GetName()) != std::string::npos);
    bool isIred2 = (sensorName == prefix + IRED2_CAMERA_NAME) ||
                   (sensorName.find("::" + std::string(IRED2_CAMERA_NAME)) != std::string::npos &&
                    sensorName.find(_model->GetName()) != std::string::npos);
    bool isColor = (sensorName == prefix + COLOR_CAMERA_NAME) ||
                   (sensorName.find("::" + std::string(COLOR_CAMERA_NAME)) != std::string::npos &&
                    sensorName.find(_model->GetName()) != std::string::npos);
